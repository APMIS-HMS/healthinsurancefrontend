...
import {BeneficiaryService, FacilityService, SystemModuleService} from './../../../services/index';
...
export class NewCheckinComponent implements OnInit {
  ...
  constructor(
      ...
      private _uploadService: UploadService, private _toast: ToastsManager) {
  ...
  ngOnInit() {
    ...
    this.listsearchControl.valueChanges.debounceTime(350)
  ...
  _getBeneficiariesFromPolicy(platformId, search) {
    if (search.length > 2) {
      this._policyService.searchPolicy(search)
          .then((payload: any) => {
            this.beneficiaries = [];
  ...
  routeBeneficiaryDetails(beneficiary) {
    if (!beneficiary.policy.isPaid || !beneficiary.policy.isActive) {
      ...
      this._router
          .navigate(
              ['/modules/beneficiary/beneficiaries/' + beneficiary._id +
               '/checkin-generate'])
          .then(payload => {
            console.log(payload);
            this._systemService.off();
