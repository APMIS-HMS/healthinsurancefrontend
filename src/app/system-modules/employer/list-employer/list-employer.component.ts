...
export class ListEmployerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //     this._facilityService.find({ query: { 'facilityType._id':
    //     this.selectedUserType._id, $limit: this.limit } }).then((payload1:
    //     any) => {
    //       this.employers = payload1.data.filter(function (item) {
    //         return
    //         (item.employer.industry.name.toLowerCase().includes(payload.toLowerCase()))
    //       });
    //     });

    //     if (payload == "All") {
    //       this._facilityService.find({ query: { 'facilityType._id':
    //       this.selectedUserType._id, $limit: this.limit } }).then((payload2:
    //       any) => {
  ...
  private _getCurrentPlatform() {
    this._facilityService.find({query: {shortName: this.currentPlatform}})
        .then((res: any) => {
          if (res.data.length > 0) {
