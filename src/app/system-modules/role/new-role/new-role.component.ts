...
export class NewRoleComponent implements OnInit {
  ...
  constructor(
      ...
      private _toastr: ToastsManager, private _moduleService: ModuleService,
  ...
  checkForExistence(accessibility) {
    return this.roles.filter(x => x.accessibility._id === accessibility._id)
               .length > 0;
  }
  accessbilityChange($event, access) {
    ...
    if (isExisting && !checked) {
      const index = this.roles.findIndex(
          x => x.accessibility.accessibility._id === access.accessibility._id);
      this.roles.splice(index, 1);
    } else if (!isExisting) {
      this.roles.push(
          {module: this.selectedModule, accessibility: access.accessibility});
    }
  }

  remove(accessibility) {
    const index = this.roles.findIndex(
        x => x.accessibility.accessibility._id === accessibility._id);
    this.roles.splice(index, 1);
    this.unCheckedAccessibility(accessibility);
  }
  unCheckedAccessibility(accessibility) {
    const index = this.accessibilities.findIndex(
        x => x.accessibility._id === accessibility._id);
  ...
  onClickAddRole(valid, value) {
    if (valid) {
      ...
      if (this.selectedRole._id === undefined) {
        ...
        this.roles.forEach(item => {
          if (item.module.accessibilities !== undefined) {
            ...
            newRole.accessibilities.push(
                {accessibility: item.accessibility, module: item.module});
          }
        });
        this._roleService.create(newRole)
            .then(payload => {
              ...
              this._toastr.success(
                  'Role has been created successfully!', 'Success!');
              this._router.navigate(['/modules/role/roles'])
                  .then(res => {
                    this._systemService.off();
                  })
                  .catch(error => {
        ...
        this._roleService.update(this.selectedRole)
            .then(payload => {
              this.roleFormGroup.reset();
              this.roles = [];
              this.accessibilities = [];
