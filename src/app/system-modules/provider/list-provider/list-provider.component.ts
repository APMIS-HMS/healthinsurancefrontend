...
export class ListProviderComponent implements OnInit {
        ...
        this._facilityService
          ...
          .then((payload1: any) => {
            this.providers = payload1.data.filter(function(item) {
              return item.provider.facilityType.name
                .toLowerCase()
        ...
        this.providers = payload.data.filter(function(item) {
          try {
            return (
              ...
              item.provider.facilityType.name
                .toLowerCase()
                .includes(strVal.toLowerCase()) ||
              item.provider.facilityClass
                .toLowerCase()
                .includes(strVal.toLowerCase()) ||
  ...
  navigateToDetails(id: string) {
    this._systemService.on();
    this._router
      .navigate(["/modules/provider/providers/" + id])
      .then(res => {
        this._systemService.off();
      })
      .catch(err => {
