...
export class PaymentDetailBeneficiaryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._headerEventEmitter.setMinorRouteUrl(
      "Payment and payment history for beneficiary"
    );
    ...
    this.paymentOptionGroup.controls["paymentOption"].valueChanges.subscribe(
      value => {
        console.log(value);
  ...
  paymentDone(data) {
    ...
    if (this.payment === "flutterwave") {
      if (
        ...
        data.tx.chargeResponseCode === "02" ||
        data.tx.chargeResponseCode === "2"
      ) {
        // Declined payment
  ...
  onClickCreateAndPaybatch(valid: boolean, value: any) {
    if (valid) {
      ...
      this._premiumPaymentService
        .payWidthCashWithMiddleWare(ref)
        .then((res: any) => {
          console.log(res);
          if (!!res) {
  ...
  createPremium(ref) {
    ...
    this._premiumPaymentService
      ...
      .then((res: any) => {
        ...
        this._premiumPaymentService
          .verifyPaymentWithMiddleWare(verificationData)
          .then((verifyRes: any) => {
