...
import {SPONSORSHIP, SPONSORSHIPTYPE} from '../../../../services/globals/config';
...
export class NewBeneficiaryProgramComponent implements OnInit {
        ...
        value => {
          if (this.filteredPremiumPackages.length > 0) {
            this.premiumPackages = this.filteredPremiumPackages.filter(
                e =>
                    e.category.name.toLowerCase() === value.name.toLowerCase());
          }
        });
  ...
  _getPlanByType(id) {
    this._systemService.on();
    this._planService.find({query: {'planType._id': id}})
        .then((res: any) => {
          this._systemService.off();
          this.plans = res.data;
        })
    ...
    this._facilityService
        ...
        .then((res: any) => {
          ...
          if (this.user.userType.name === 'Health Insurance Agent') {
            this.isHIA = true;
            let index =
                this.hias.findIndex(x => x._id === this.user.facilityId._id);
            if (index > -1) {
              this.frmProgram.controls.hiaName.setValue(this.hias[index]);
              const hia = this.frmProgram.controls.hiaName.value;
