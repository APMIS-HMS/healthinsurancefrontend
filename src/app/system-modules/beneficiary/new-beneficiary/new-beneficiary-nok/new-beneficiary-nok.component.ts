...
export class NewBeneficiaryNokComponent implements OnInit {
  ...
  closeDependant(dependant, i) {
    if (
      (<FormArray>this.frmNok.controls["dependantArray"]).controls.length > 1
    ) {
  ...
  pushNewDependant(dependant?, index?) {
    ...
    (<FormArray>this.frmNok.controls["dependantArray"]).push(
      this._fb.group({
        firstName: ["", [<any>Validators.required]],
        title: ["", [<any>Validators.required]],
        middleName: [""],
  ...
  canProceed() {
    return (
      this.frmNok["controls"].dependantArray["controls"].filter(
