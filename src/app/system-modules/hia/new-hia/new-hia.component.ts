...
import {Address, BankDetail, Beneficiary, Contact, Facility, Gender, Hia, MaritalStatus, Person, Title} from '../../../models/index';
...
import {BankService, ContactPositionService, CountryService, FacilityService, HiaGradeService, HiaTypeService, SystemModuleService, UserTypeService} from './../../../services/index';
...
export class NewHiaComponent implements OnInit {
  ...
  constructor(
      ...
      private _route: ActivatedRoute, private _uploadService: UploadService,
      private _router: Router, private _hiaTypeService: HiaTypeService,
      private _hiaGradeService: HiaGradeService) {}
  ngOnInit() {
    this._headerEventEmitter.setRouteUrl('New HIA');
    this._headerEventEmitter.setMinorRouteUrl(
        'Create New Health Insurance Agent');
  ...
  _getHiaTypes() {
    ...
    this._hiaTypeService.find({query: {$limit: 200}})
        .then((payload: any) => {
  ...
  _getHiaGrades() {
    this._systemService.on();
    this._hiaGradeService.find({query: {$limit: 200}})
        .then((payload: any) => {
          this._systemService.off();
  ...
  _initialiseFormGroup() {
    let date = this.selectedFacility.hia === undefined ?
        new Date() :
        new Date(this.selectedFacility.hia.registrationDate);
    ...
    this.hiaFormGroup = this._fb.group({
      ...
      phone: [
        this.selectedFacility != null ? this.selectedFacility.phoneNumber : '',
      ...
      type: [
        this.selectedFacility.hia != null ? this.selectedFacility.hia.type : '',
        [<any>Validators.required]
      ],
      grade: [
        this.selectedFacility.hia != null ? this.selectedFacility.hia.grade :
                                            '',
        [<any>Validators.required]
      ],
      facilityType: [
        this.selectedFacility != null ? this.selectedFacility.facilityType : '',
        [<any>Validators.required]
      ],
      bank: [
        this.selectedFacility.address != null ?
      ...
      nhisNumber: [
        this.selectedFacility.hia != null ?
            this.selectedFacility.hia.nhisNumber :
            '',
        [<any>Validators.required]
      ],
      // cinNumber: [this.selectedFacility.hia != null ?
      // this.selectedFacility.hia.cin : '', [<any>Validators.required]],
      registrationDate: [
        this.selectedFacility.hia != null ?
            this.selectedFacility.hia.registrationDate :
            this.today,
        [<any>Validators.required]
      ]
    });
    ...
    if (this.selectedFacility.name !== undefined) {
      ...
      //   this._uploadService.transform(this.selectedFacility.logo.thumbnail);
      // }
    }
  ...
  _extractBusinessContact(businessContact?: Contact) {
    ...
    businessContact.lastName =
        this.hiaFormGroup.controls['businessContactName'].value;
    businessContact.firstName =
        this.hiaFormGroup.controls['businessContactFirstName'].value;
    businessContact.email =
        this.hiaFormGroup.controls['businessContactEmail'].value;
    businessContact.phoneNumber =
        this.hiaFormGroup.controls['businessContactNumber'].value;
    businessContact.position =
        this.hiaFormGroup.controls['businessContactPosition'].value;
  ...
  _extractITContact(itContact?: Contact) {
    ...
    itContact.firstName =
        this.hiaFormGroup.controls['iTContactFirstName'].value;
  ...
  _extractBankDetail(bankDetail?: BankDetail) {
    ...
    bankDetail.accountNumber =
        this.hiaFormGroup.controls['bankAccNumber'].value;
  ...
  _extractHIA(hia?: HIA) {
    ...
    hia.registrationDate =
        this.hiaFormGroup.controls['registrationDate'].value.jsdate;
    return hia;
  }
  ...
  _extractFacility(facility?: Facility) {
    if (facility === undefined) {
      ...
      const businessContact =
          this._extractBusinessContact(facility.businessContact);
  ...
  save(valid, value) {
    if (valid) {
      ...
      let facility = this.selectedFacility._id === undefined ?
          this._extractFacility() :
          this._extractFacility(this.selectedFacility)
      if (facility._id === undefined) {
              ...
              this._facilityService.create(facility)
                  .then((payload: Facility) => {
                    ...
                    this._toastr.success(
                        'Health Insurance Agent has been created successfully!',
                        'Success!');
        ...
        //     this._systemService.off();
        //     // this.hiaFormGroup.reset();
        //     this.saveBtn = "SAVE &nbsp; <i class='fa fa-check'
        //     aria-hidden='true'></i>"; this._toastr.success('Health Insurance
        //     Agent has been created successfully!', 'Success!');
        //     this._router.navigate(['/modules/hia/hias']);
        //   }).catch(err => {
        //     console.log(err)
        //     this._systemService.off();
        ...
        (payload: any) => {
          ...
          if (payload.data.length > 0) {
            ...
            const index = payload.data.findIndex(
                x => x.name === 'Health Insurance Agent');
            if (index > -1) {
              this.selectedUserType = payload.data[index];
              this.hiaFormGroup.controls['facilityType'].setValue(
